...
function Holyquran() {
  ...
  const categoryData = data?.find(book => book.category === "holyquran");

  // const allBooks = categoryData?.subcategories || [];
  const allBooks = categoryData?.subcategories?.flatMap(sub => sub.books) || [];

  const booksToRender = display ? allBooks.slice(0, 12) : null;
    ...
    <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 px-4 lg:px-30">
        {booksToRender.map((book) => (
          ...
          >
            {/* Image */}
            <Link to={`/products/holyquran/${book.id}`}>
